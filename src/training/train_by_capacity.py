...
def train_single_capacity(capacity: str, verbose: bool = True):
    ...
    base_model_path = "results/prabayar/models/model_prabayar.json"
    capacities = ["450", "900", "1300", "2500", "3300"]
    if capacity in capacities and os.path.exists(base_model_path):
        if verbose:
            print(f"\n [Transfer Learning] Loading base model weights...")
        try:
            model.load_weights_from(base_model_path, freeze_layers=2)
            if verbose:
                print("Weights loaded successfully. First 2 layers frozen.")
        except Exception as e:
            if verbose:
                print(f"      [Warning] Transfer learning failed: {e}")
